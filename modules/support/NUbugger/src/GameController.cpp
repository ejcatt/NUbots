...
namespace support {
    ...
    void NUbugger::provideGameController() {
        ...
        handles["game_controller"].push_back(on<Trigger<TeamColour>, With<GameState>>().then([this](const TeamColour&, const GameState& gameState) {
            sendGameState("TeamColour", gameState);
        }));

        handles["game_controller"].push_back(on<Trigger<Score>, With<GameState>>().then([this](const Score&, const GameState& gameState) {
            sendGameState("Score", gameState);
        }));

        handles["game_controller"].push_back(on<Trigger<GoalScored<TEAM>>, With<GameState>>().then([this](const GoalScored<TEAM>&, const GameState& gameState) {
            sendGameState("GoalScored<TEAM>", gameState);
        }));

        handles["game_controller"].push_back(on<Trigger<GoalScored<OPPONENT>>, With<GameState>>().then([this](const GoalScored<OPPONENT>&, const GameState& gameState) {
            sendGameState("GoalScored<OPPONENT>", gameState);
        }));

        handles["game_controller"].push_back(on<Trigger<Penalisation<SELF>>, With<GameState>>().then([this](const Penalisation<SELF>&, const GameState& gameState) {
            sendGameState("Penalisation<SELF>", gameState);
        }));

        handles["game_controller"].push_back(on<Trigger<Penalisation<TEAM>>, With<GameState>>().then([this](const Penalisation<TEAM>&, const GameState& gameState) {
            sendGameState("Penalisation<TEAM>", gameState);
        }));

        handles["game_controller"].push_back(on<Trigger<Penalisation<OPPONENT>>, With<GameState>>().then([this](const Penalisation<OPPONENT>&, const GameState& gameState) {
            sendGameState("Penalisation<OPPONENT>", gameState);
        }));

        handles["game_controller"].push_back(on<Trigger<Unpenalisation<SELF>>, With<GameState>>().then([this](const Unpenalisation<SELF>&, const GameState& gameState) {
            sendGameState("Unpenalisation<SELF>", gameState);
        }));

        handles["game_controller"].push_back(on<Trigger<Unpenalisation<TEAM>>, With<GameState>>().then([this](const Unpenalisation<TEAM>&, const GameState& gameState) {
            sendGameState("Unpenalisation<TEAM>", gameState);
        }));

        handles["game_controller"].push_back(on<Trigger<Unpenalisation<OPPONENT>>, With<GameState>>().then([this](const Unpenalisation<OPPONENT>&, const GameState& gameState) {
            sendGameState("Unpenalisation<OPPONENT>", gameState);
        }));

        handles["game_controller"].push_back(on<Trigger<CoachMessage<TEAM>>, With<GameState>>().then([this](const CoachMessage<TEAM>&, const GameState& gameState) {
            sendGameState("CoachMessage<TEAM>", gameState);
        }));

        handles["game_controller"].push_back(on<Trigger<CoachMessage<OPPONENT>>, With<GameState>>().then([this](const CoachMessage<OPPONENT>&, const GameState& gameState) {
            sendGameState("CoachMessage<OPPONENT>", gameState);
        }));

        handles["game_controller"].push_back(on<Trigger<HalfTime>, With<GameState>>().then([this](const HalfTime&, const GameState& gameState) {
            sendGameState("HalfTime", gameState);
        }));

        handles["game_controller"].push_back(on<Trigger<BallKickedOut<TEAM>>, With<GameState>>().then([this](const BallKickedOut<TEAM>&, const GameState& gameState) {
            sendGameState("BallKickedOut<TEAM>", gameState);
        }));

        handles["game_controller"].push_back(on<Trigger<BallKickedOut<OPPONENT>>, With<GameState>>().then([this](const BallKickedOut<OPPONENT>&, const GameState& gameState) {
            sendGameState("BallKickedOut<OPPONENT>", gameState);
        }));

        handles["game_controller"].push_back(on<Trigger<KickOffTeam>, With<GameState>>().then([this](const KickOffTeam&, const GameState& gameState) {
            sendGameState("KickOffTeam", gameState);
        }));

        handles["game_controller"].push_back(on<Trigger<GamePhase<Phase::INITIAL>>, With<GameState>>().then([this](const GamePhase<Phase::INITIAL>&, const GameState& gameState) {
            sendGameState("GamePhase<Phase::INITIAL>", gameState);
        }));

        handles["game_controller"].push_back(on<Trigger<GamePhase<Phase::READY>>, With<GameState>>().then([this](const GamePhase<Phase::READY>&, const GameState& gameState) {
            sendGameState("GamePhase<Phase::READY>", gameState);
        }));

        handles["game_controller"].push_back(on<Trigger<GamePhase<Phase::SET>>, With<GameState>>().then([this](const GamePhase<Phase::SET>&, const GameState& gameState) {
            sendGameState("GamePhase<Phase::SET>", gameState);
        }));

        handles["game_controller"].push_back(on<Trigger<GamePhase<Phase::PLAYING>>, With<GameState>>().then([this](const GamePhase<Phase::PLAYING>&, const GameState& gameState) {
            sendGameState("GamePhase<Phase::PLAYING>", gameState);
        }));

        handles["game_controller"].push_back(on<Trigger<GamePhase<Phase::TIMEOUT>>, With<GameState>>().then([this](const GamePhase<Phase::TIMEOUT>&, const GameState& gameState) {
            sendGameState("GamePhase<Phase::TIMEOUT>", gameState);
        }));

        handles["game_controller"].push_back(on<Trigger<GamePhase<Phase::FINISHED>>, With<GameState>>().then([this](const GamePhase<Phase::FINISHED>&, const GameState& gameState) {
            sendGameState("GamePhase<Phase::FINISHED>", gameState);
        }));

        handles["game_controller"].push_back(on<Trigger<GameMode<Mode::NORMAL>>, With<GameState>>().then([this](const GameMode<Mode::NORMAL>&, const GameState& gameState) {
            sendGameState("GameMode<Mode::NORMAL>", gameState);
        }));

        handles["game_controller"].push_back(on<Trigger<GameMode<Mode::PENALTY_SHOOTOUT>>, With<GameState>>().then([this](const GameMode<Mode::PENALTY_SHOOTOUT>&, const GameState& gameState) {
            sendGameState("GameMode<Mode::PENALTY_SHOOTOUT>", gameState);
        }));

        handles["game_controller"].push_back(on<Trigger<GameMode<Mode::OVERTIME>>, With<GameState>>().then([this](const GameMode<Mode::OVERTIME>&, const GameState& gameState) {
