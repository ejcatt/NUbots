...
namespace support {
    ...
    NUcap::NUcap(std::unique_ptr<NUClear::Environment> environment)
        ...
        on<Trigger<Configuration<NUcap> > >().then([this](const Configuration<NUcap>& config) {
        ...
        on<Trigger<Every<15, Per<std::chrono::seconds>>>>().then([this](const time_t&) {
            bool valid;
