...
namespace modules {
    namespace audio {
        ...
        BeatDetector::BeatDetector(std::unique_ptr<NUClear::Environment> environment) : Reactor(std::move(environment)) {
            ...
            on<Trigger<Last<2, Buckets>>>().then([this](const std::vector<std::shared_ptr<const Buckets>>& buckets) {
            ...
            on<Trigger<Last<NUM_BEAT_SAMPLES, DiffBuckets>>>().then([this](const std::vector<std::shared_ptr<const DiffBuckets>>& input) {
            ...
            on<Trigger<Last<100, Beat>>>().then([this](const std::vector<std::shared_ptr<const Beat>>& input) {
            ...
            on<Trigger<Last<2, FilteredBeat>>>().then([this](const std::vector<std::shared_ptr<const FilteredBeat>>& input) {
